...
class MonthTabsRouter extends React.Component {
  ...
  render(){
    if(this.props.tabId == 'All'){
      return <Link to={{pathname: '/trendywords', search: '?month=All&year='+this.props.year}} >
        <p style={this.state.style}>Show All</p>
        </Link>
    }
    else{
      return <Link to={{pathname: '/trendywords', search: '?month='+this.props.tabId + '&year='+this.props.year }} >
